...
To improve answer quality, we implemented a **Contextual Processor** that adds document-level context to each chunk before indexing. This method, `add_context_to_chunk`, generates a succinct summary for each document and prepends it to every chunk. This reduces retrieval ambiguity and significantly improves answer relevance, especially for broad queries like “What do you know about Alexandria?”

- **Location:** `rag/contextual_processor.py`  
- **Purpose:** Prepends LLM-generated document context to chunks for better semantic retrieval.
